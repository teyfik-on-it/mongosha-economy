...
const log = (message) => {
  console.log(
    `${bgLoad(`[${moment().format('YYYY-MM-DD HH:mm:ss')}]`)} ${message}`,
  );
};
...
fs.readdir('./commands/', (err, files) => {
  ...
  files.forEach((f) => {
    fs.readdir(`./commands/${f}/`, (err, filess) => {
      ...
      log(
        load(
          `${filess.length} command will be loaded from commands/${f} folder.`,
        ),
      );
      filess.forEach((fs) => {
        const props = require(`./commands/${f}/${fs}`);
...
DatabaseManager.connect(settings.botSettings.mongoURL).then(async () => {
  client
    .login(settings.botSettings.token)
    .then(() => {
      console.log(
        bgLoad(`[${moment().format('YYYY-MM-DD HH:mm:ss')}]`) +
          token(` Successfully connected to the token.`),
      );
    })
    .catch((err) => {
      console.log(
        bgLoad(`[${moment().format('YYYY-MM-DD HH:mm:ss')}]`) +
          error(` Couldn't connect to token! ${err}`),
      );
    });
});
