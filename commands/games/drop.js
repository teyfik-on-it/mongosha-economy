...
module.exports.run = async (client, message, args) => {
  ...
  const timeout = 10000;
  const bump = await db.get(`cooldowns.drop.${message.author.id}`);
  if (bump !== null && timeout - (Date.now() - bump) > 0) {
    const time = ms(timeout - (Date.now() - bump));
    message.channel.send(
      `Hurra! Yerden __bu kadar hızlı__ para toplayamazsın **${message.author.username}**!. Lütfen ${time.hours} saat, ${time.minutes} dakika, ${time.seconds} saniye bekle.`,
    );
  ...
  if (!args[0]) {
    message.channel.send(
      `Hurra! Bir para miktarı girmelisin **${message.author.username}**!`,
    );
    message.react(denymoji);
    return;
  }
  ...
  const totalmoney =
    (await db.get(`bank.games.drop.funds.${message.channel.id}`)) || 0;
  ...
  if (totalmoney > args[0]) {
    ...
    message.channel
      .send(
        `${dropped} Hurra! Şanslı günündesin **${message.author.username}**! Yerden toplamda **\`${args[0]}\` SC** ele geçirdin.`,
      )
      .then((x) => x.delete({ timeout: 5000 }));
  ...
  if (totalmoney < args[0]) {
    ...
    message.channel
      .send(
        `${dropped} Tüh ya! Hiç şanslı değilsin **${message.author.username}**! Yere toplamda **\`${args[0]}\' SC** düşürdün.`,
      )
...
module.exports.help = {
  name: 'düşür',
  help:
    'Düşürme oyununda toplamak için girdiğiniz bakiye eğer yerde yoksa siz düşürürsünüz.',
  usage: 'düşür [<bahis>]',
  category: 'Games',
};
