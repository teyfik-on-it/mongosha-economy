...
module.exports.run = async (client, message, args) => {
  const checke = await db.get(`bank.account.${message.author.id}`);
  if (!checke) {
  ...
  const check = await db.get(`bank.properties.${message.author.id}.rings`);
  ...
  if (args[0] === 'yüzük') {
    if (args[1] === 'al') {
      if (args[2] === 'AAAA') {
        if (balance < 100)
          return message.channel.send(
            `Hey! Şu an zaten bankanda \`${balance}\` .var`,
          );

        await db.add(`bank.${membercode}.funds`, -100);
        await db.push(`bank.properties.${message.author.id}.rings`, 'AAAA');
        message.channel.send(
          `Seni ne kadar idare eder bilmiyorum... Al bakalım <:lv1:799562415548661770>`,
        );
        return;
      }
      ...
      if (args[2] === 'AAAB') {
        ...
        message.channel.send(
          `Yüzük kısmını daha yeni yapmıştım. Sıcak olabilir dikkat et <:lv2:799562419474268200>`,
        );
        return;
      }
      ...
      if (args[2] === 'AAAC') {
        ...
        message.channel.send(
          `Maliyetten kıstığımı kimseye söyleme <:lv3:799562443646566411>`,
        );
        return;
      }
      ...
      if (args[2] === 'AAAD') {
        ...
        message.channel.send(
          `Normal demir dökümden daha sağlam yaptım <:lv4:799562445231751178>`,
        );
        return;
      }
      ...
      if (args[2] === 'AAAE') {
        ...
        message.channel.send(
          `Normalde platini pek sevmiyorum fakat yine de nasıl istiyorsan öyle olsun <:lv5:799562445467025420>`,
        );
        return;
      }
      ...
      if (args[2] === 'AAAF') {
        ...
        message.channel.send(
          `Yaldız normalde pek iyi değil ya ama yine de al bakalım <:lv6:799562445621559337>`,
        );
        return;
      }
      ...
      if (args[2] === 'AAAG') {
        ...
        message.channel.send(
          `Parıl parıl parlıyor kafam karışıyor. Ne olduğunu bilmediğim bir maddeden yaptım <:lv7:799562446264205342>`,
        );
        return;
      }
      ...
      if (args[2] === 'AAAH') {
        ...
        message.channel.send(
          `Fasilyus gerçekten namert bir hırsızdı. Onun yüzüğünü çalanın hayat iksiri elde ettiğine dair sözler var <:lv8:799562437992120320>`,
        );
        return;
      }
      ...
      if (args[2] === 'AAAI') {
        ...
        message.channel.send(
          `Konfiçyus bilgeliğiyle tanınan birisiydi. Onun yüzüğünde hayatın bütün sırlarını bulabilirsin <:lv9:799562436487020554>`,
        );
        return;
      }
      ...
      if (args[2] === 'AABA') {
        ...
        message.channel.send(
          `Fazilet çok çekti, çok yaşadı ve çok gördü. Buna değdi mi <:lv10:799562439355138080>`,
        );
        return;
      }
    }
    ...
    const lv11 = client.emojis.cache.get(emojis.rings.lv1);
    const lv12 = client.emojis.cache.get(emojis.rings.lv2);
    const lv13 = client.emojis.cache.get(emojis.rings.lv3);
    const lv14 = client.emojis.cache.get(emojis.rings.lv4);
    const lv15 = client.emojis.cache.get(emojis.rings.lv5);
    const lv16 = client.emojis.cache.get(emojis.rings.lv6);
    const lv17 = client.emojis.cache.get(emojis.rings.lv7);
    const lv18 = client.emojis.cache.get(emojis.rings.lv8);
    const lv19 = client.emojis.cache.get(emojis.rings.lv9);
    const lv110 = client.emojis.cache.get(emojis.rings.lv10);

    const rings = new MessageEmbed()
      .setAuthor(
        message.author.tag,
      ...
      .setDescription(
        `Pheh, pheh! Ben yükseklerin lideri \`March Dukke\`. Evet ben bir düktüm ama artık sadece yüzük satıcısıyım. Her neyse, sana nasıl yardımcı olabilirim?
 ...
 `,
  ...
  if (args[0] === 'unvan') {
    if (args[1] === 'al') {
      if (args[2] === 'AABB') {
        ...
        const checking = await db.get(
          `bank.properties.${message.author.id}.titles.owned`,
        );
        if (checking !== undefined) {
          const check = Object.values(checking);
          if (check.includes('AABB'))
            return message.channel.send(
              `Sen bu unvanı zaten daha önce almışsın.`,
            );
        }
        await db.push(
          `bank.properties.${message.author.id}.titles.owned`,
        ...
        message.channel.send(
          `Galiba buralarda yenisin? Al bakalım <:rozetother:799603119733997590>`,
        );
        return;
      }
      ...
      if (args[2] === 'AABC') {
        ...
        message.channel.send(
          `Bu unvanı en son alan kişi ölmüştü ama sana ne olur bilemem <:rozetdefault:799603118878883840>`,
        );
        return;
      }
      ...
      if (args[2] === 'AABD') {
        ...
        message.channel.send(
          `Normalde fazla bir değeri yok ama benim için özel <:rozetdukke:799603125886386198>`,
        );
        return;
      }
      ...
      if (args[2] === 'AABE') {
        ...
        message.channel.send(
          `Garip bir hikayesi var okumak istersen <:rozetsevgici:799603126805856256>`,
        );
        return;
      }
      ...
      if (args[2] === 'AABF') {
        ...
        message.channel.send(
          new MessageEmbed()
            ...
            .setDescription(
              `Başarıyla **Onaylı Satıcı** durumunu aktifleştirdin. Artık profilin değişti ve daha fazla tutarda para aktarımı yapabiliyorsun.`,
            )
            .setColor('GOLD'),
        );
        return;
    ...
    if (args[1] === 'görüntüle') {
      ...
      const array = await db.get(`bank.properties.${member.id}.titles.list`);
      console.log(array);
      return;
    }
  ...
  const informatcja = new MessageEmbed()
    .setAuthor(message.author.tag, message.author.avatarURL({ dynamic: true }))
    .setTitle(`Büyük Market`)
    .setDescription(
      `Markete hoş geldin! Ben yardımcı \`Noeul\`. Sana bir şeyler satmak için burdayım. Para nakit demektir ve nakiti herkes sever değil mi? Lanet olası haşereler evimi basmadan önce daha fazla eşyam olduğuna yemin edebilirim. Ne diyorduk? Ha işte senin için market listesi.
