...
module.exports.run = async (client, message, args) => {
  ...
  const moneymoji = client.emojis.cache.get(emojis.other.money);

  const timeout = 86400000;
  const bump = await db.get(`cooldowns.daily.${message.author.id}`);
  if (bump !== null && timeout - (Date.now() - bump) > 0) {
    const time = ms(timeout - (Date.now() - bump));
    return message.channel.send(
      `${moneymoji} Hurra! Günlük maaşını toplaman için süren bitmedi **${message.author.username}**! Lütfen ${time.hours} saat, ${time.minutes} dakika, ${time.seconds} saniye bekle.`,
    );
  }

  const membercode = await db.get(`bank.account.${message.author.id}`);

  const money = Math.floor(Math.random() * 1550);
  ...
  message.channel.send(
    new MessageEmbed()
      ...
      .setDescription(
        `${moneymoji} **${message.author.username}** günlük maaşın olan **${money} SC**'yi başarıyla topladın.`,
      )
      .setColor('EC6969'),
  );
